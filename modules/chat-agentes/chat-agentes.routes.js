...
const { uploadChatArchivos } = require("../../config/upload");
...
router.get("/chat-agentes", requireAuth, async (req, res) => {
    try {
...
router.get("/chat-conversaciones", requireAuth, async (req, res) => {
    try {
        if (!(await requireRRHH(req, res))) return;
...
router.post("/chat-conversaciones", requireAuth, async (req, res) => {
    try {
        if (!(await requireRRHH(req, res))) return;
...
router.get("/chat-conversaciones/:id", requireAuth, async (req, res) => {
    try {
        if (!(await requireRRHH(req, res))) return;
...
router.delete("/chat-conversaciones/:id", requireAuth, async (req, res) => {
    try {
        if (!(await requireRRHH(req, res))) return;
        await pool.query(
...
router.post("/chat-conversaciones/:id/mensajes", requireAuth, uploadChatArchivos.array('archivos'), async (req, res) => {
    try {
        if (!(await requireRRHH(req, res))) return;
        const conv = await pool.query(
            "SELECT * FROM web.chat_agente_conversacion WHERE id = $1 AND usuario = $2",
